...
const SECTION_KW_WEIGHT = {
  skills: 1.0, 'technical skills': 1.0, technologies: 1.0, 'tech stack': 1.0,
  'tools and technologies': 1.0, 'core competencies': 1.0, competencies: 1.0,
  experience: 0.9, 'work experience': 0.9, 'professional experience': 0.9,
  'employment history': 0.9,
  projects: 0.8, 'personal projects': 0.8, 'key projects': 0.8,
  summary: 0.75, profile: 0.75, top: 0.70,
  education: 0.45,
  certifications: 0.6, achievements: 0.6,
  activities: 0.25, interests: 0.20, hobbies: 0.15, languages: 0.25,
};
const DEFAULT_SECTION_WEIGHT = 0.50;

// ── Strong action verbs ────────────────────────────────────────────────────
const STRONG_VERBS = new Set([
  'accelerated','achieved','architected','automated','built','championed',
  'consolidated','created','cut','decreased','defined','delivered','deployed',
  'designed','developed','devised','directed','drove','eliminated','enabled',
  'engineered','enhanced','established','exceeded','executed','expanded',
  'generated','grew','identified','implemented','improved','increased',
  'innovated','integrated','launched','maintained','mentored','migrated',
  'modernized','optimized','orchestrated','overhauled','pioneered','produced',
  'reduced','refactored','released','revamped','scaled','shipped','simplified',
  'slashed','solved','spearheaded','standardized','streamlined','strengthened',
  'supervised','trained','transformed','upgraded','delivered','negotiated',
  'resolved','restructured','revitalized','secured','shaped','sourced',
]);

// ── Weak openers ──────────────────────────────────────────────────────────
const WEAK_OPENERS = [
  /^responsible\s+for\b/i, /^assisted?\s+(with|in)\b/i,
  /^helped?\s+(with|to)\b/i, /^worked\s+on\b/i,
  /^duties\s+(include|included|were)\b/i, /^was\s+involved\s+in\b/i,
  /^participated\s+in\b/i, /^part\s+of\s+(a\s+|the\s+)?(team|group)\b/i,
  /^contributed\s+to\b/i, /^served\s+as\b/i, /^tasked\s+with\b/i,
  /^handled\b/i, /^did\b/i, /^made\b/i, /^in\s+charge\s+of\b/i,
  /^involved\s+in\b/i, /^support(ed|ing)\b/i,
];
...
const BUZZWORDS = [
  ...
  'passionate', 'passionate about', 'enthusiasm', 'enthusiastic',
  'dynamic', 'results-driven', 'results driven', 'results-oriented',
  'go-to person', 'thought leader', 'thought leadership',
  'change agent', 'game changer', 'game-changer', 'disruptive',
  'strategic thinker', 'visionary', 'proactive', 'self-starter',
  'team player', 'hardworking', 'hard-working', 'hard working',
  'detail-oriented', 'detail oriented', 'motivated',
  'strong work ethic', 'people person', 'outside the box',
  'value-add', 'move the needle', 'deep dive', 'bandwidth',
  'circle back', 'touch base', 'scalable solution', 'holistic approach',
  'bleeding edge', 'pain point', 'pain points', 'low-hanging fruit',
  'robust solution', 'seamless', 'empower', 'empowering',
...
const WEAK_VERB_PATTERNS = [
  ...
  /\bhave\s+(been|had)\b/i, /\bhad\s+to\b/i,
  /\btried\s+to\b/i, /\battempted\s+to\b/i,
  /\blearned\s+about\b/i, /\bgained\s+experience\b/i,
  /\bexposure\s+to\b/i, /\bfamiliar\s+with\b/i,
  /\bknowledge\s+of\b/i, /\bbasic\s+knowledge\b/i,
...
const GROWTH_SIGNALS = {
  ...
  award:        /\b(award(ed)?|recogni(zed|sed|tion)|honor(ed)?|scholarship|fellowship|prize|winner|ranked\s+(first|1st|top|\d+)|dean'?s\s+list|hackathon|competition)\b/i,
  // Expanded: catches "team of 8+", "led 5 members", "coordinated 15+ events"
  leadership:   /\b(led\s+(a\s+)?(team\s+of\s*)?\d+|managed\s+\d+\s*(people|engineer|member|student)|supervised\s+\d+|mentored\s+\d+|team\s+of\s*\d+|coordinated\s+\d+|organized\s+\d+|trained\s+\d+)\b/i,
  // Expanded: catches "improving X by N%", "efficiency by N%", "reducing X by N%"
  impact:       /\b(revenue|cost\s+sav(ing|ed)|reduced\s+cost|saved\s+[\$₹€]|generated\s+[\$₹€]|roi|profit|growth\s+of\s+\d+%|efficiency\s+(by|of)\s+\d+%|improv\w+\s+\w+\s+by\s+\d+%|reduc\w+\s+\w+\s+by\s+\d+%|increas\w+\s+\w+\s+by\s+\d+%|by\s+\d+%)\b/i,
  contribution: /\b(published\s+[a-z]|open.?sourced?|contributed\s+to\s+(?!the\b|our\b|this\b)|npm\s+package|pypi|launched\s+(a\s+)?product|github\s+stars?)\b/i,
  // NEW: student-specific — N+ participants, users, members, events
  scale:        /\b(\d+\+?\s*(participants?|attendees?|students?|members?|users?|customers?|events?|institutions?|organizations?|teams?))\b/i,
};

// ── Niche skills — only suggest if candidate is already deep in that niche ─
const NICHE_SKILLS = new Set([
  'rust', 'assembly', 'posix', 'opengl', 'vulkan', 'cuda',
  'rtos', 'firmware', 'embedded systems', 'low-level programming',
  'winsock2', 'compiler design', 'computer architecture',
  'haskell', 'clojure', 'erlang', 'elixir',
  'databricks', 'apache hadoop',
...
function resolveToken(token) {
  ...
  if (t.length >= 5) {
    ...
    for (const skill of SINGLE_WORD_SKILLS) {
      ...
      if (sim > best) { best = sim; bestSkill = skill; }
    }
    return bestSkill;
  }
  return null;
...
//  — skip abstract/theory terms that can't be "added" to a resume naturally
//  — never show a keyword that's already matched
// ======================================================

// These are valid skills but too abstract to suggest as "missing" —
// you can't just add "distributed systems" to your resume without demonstrating it.
const ABSTRACT_SKILLS = new Set([
  'distributed systems', 'compiler design', 'computer architecture',
  'functional programming', 'solid', 'mvc', 'mvvm',
  'observer pattern', 'factory pattern', 'singleton pattern',
  'microservices architecture', 'pair programming', 'clean code',
  'discrete mathematics', 'linear algebra', 'probability', 'statistics',
  'time complexity', 'space complexity', 'competitive programming',
  'documentation', 'version control', // too generic
]);
...
function getMissingKeywords(matched, primaryDomain, secondaryDomain) {
  ...
  // Domain priority: specific domains first, generic fallbacks last
  // Avoid generic domains as primary source of suggestions
  const domains = [
    primaryDomain,
    secondaryDomain,
    'cs_fundamentals',
  ...
  for (const domain of domains) {
    ...
    for (const skill of skills) {
      ...
      // Skip abstract theory terms — not actionable suggestions
      if (ABSTRACT_SKILLS.has(skill)) continue;
...
function detectWeakVerbDensity(rawText, bullets) {
  ...
  const penalty      = density >= 0.60 ? 4 : density >= 0.40 ? 3 : density >= 0.25 ? 2 : density >= 0.10 ? 1 : 0;
...
function detectGrowthSignals(rawText, bullets, hasExperienceSection) {
  ...
  if (hasExperienceSection && bullets.length >= 4) {
    if (signalsFound.length === 0) {
      // Cap at 5 — a student with club/org experience legitimately won't have
      // promotions or awards. -7 was too punishing for this audience.
      penalty = bullets.length >= 8 ? 5 : bullets.length >= 6 ? 3 : 2;
    } else if (signalsFound.length === 1) {
      penalty = bullets.length >= 8 ? 2 : 0;
    }
  }
...
function detectRepetition(bullets) {
  ...
  for (const b of bullets) {
    const toks = b.text.toLowerCase().replace(/[^a-z\s]/g, ' ').split(/\s+/).filter(t => t.length > 2);
...
function computeKeywordsScore(matched) {
  ...
  // Thresholds tightened from original to reduce keyword inflation from
  // pure skills-list resumes, but mid-range tier (15-17) added to avoid
  // swallowing genuine strong resumes (effectiveCount ~15).
  let score;
  if      (effectiveCount >= 26) score = 35;
  else if (effectiveCount >= 21) score = 28;
  else if (effectiveCount >= 17) score = 22;
  else if (effectiveCount >= 14) score = 18;  // key tier — catches Uttej-level resumes
  else if (effectiveCount >= 10) score = 13;
  else if (effectiveCount >= 6)  score = 8;
  else if (effectiveCount >= 3)  score = 4;
...
function computeExperienceScore(parsed) {
  ...
  const expOnlyBullets = parsed.bullets.filter(b =>
    ['experience','work experience','professional experience','employment history']
      .includes((b.section || '').toLowerCase())
  );
  ...
  const nonEduBullets = parsed.bullets.filter(b =>
    !['education','academic background','qualifications'].includes((b.section || '').toLowerCase())
  );
  ...
  let verbCount = 0;
  for (const b of bullets) {
    const first = b.text.replace(/^[-*•\s]+/, '').split(/\s+/)[0]
      ?.toLowerCase().replace(/[^a-z]/g, '');
    if (first && STRONG_VERBS.has(first)) verbCount++;
  }
  const verbRatio = verbCount / total;
  let verbScore = 0;
  if      (verbRatio >= 0.80) verbScore = 10;
  else if (verbRatio >= 0.60) verbScore = 8;
  else if (verbRatio >= 0.40) verbScore = 6;
  else if (verbRatio >= 0.30) verbScore = 4;
  else if (verbRatio >= 0.20) verbScore = 2;

  if (verbRatio < 0.50 && bullets.length >= 2)
  ...
  // METRIC_RE — catches:
  //   Percentages: 40%, 100%, 30%
  //   N+ patterns: 500+ users, 50+ connections, 200+ participants
  //   Word units: 10ms, 500 users, 3 months, $1200
  //   CS complexity: O(1), O(n log n)
  const METRIC_RE = /(?:\b\d+\.?\d*[+]?\s*%|\b\d+\.?\d*[+]?\s*(?:x\b|times|hours?|days?|weeks?|months?|years?|users?|clients?|requests?|connections?|events?|participants?|members?|students?|ms\b|kb\b|mb\b|gb\b|\$|INR|Rs\.?|EUR|GBP|k\b(?!\w)|m\b(?!\w)|million|billion|engineers?|employees?|tickets?|repos?|commits?|tests?|features?|queries?|endpoints?|projects?|components?|systems?|apis?|lines?|functions?|modules?)\b|O\(1\)|O\(n\)|O\(log\s*n\)|O\(n\s*log\s*n\))/gi;

  // Exclude education bullets explicitly from metric counting
  const metricBullets_arr = bullets.filter(b =>
    !['education','academic background','qualifications'].includes((b.section || '').toLowerCase())
  );
  let metricBullets = 0, totalMetrics = 0;
  for (const b of metricBullets_arr) {
    const m = b.text.match(METRIC_RE);
    if (m) { metricBullets++; totalMetrics += m.length; }
  }
  const metricRatio = metricBullets / total;
  let metricScore = 0;
  if      (metricRatio >= 0.50 || totalMetrics >= 7) metricScore = 12;
  else if (metricRatio >= 0.35 || totalMetrics >= 5) metricScore = 10;
  else if (metricRatio >= 0.20 || totalMetrics >= 3) metricScore = 8;
  else if (metricRatio >= 0.10 || totalMetrics >= 2) metricScore = 6;
  ...
  if (weakCount > 0)
    issues.push(`${weakCount} bullet(s) use weak openers like "Responsible for" or "Worked on".`);

  // ── No experience OR projects section ─────────────────────────────────
  const lowerSecs = Object.keys(parsed.sections).map(s => s.toLowerCase());
  const hasAnyPracticalSection = lowerSecs.some(s =>
    s.includes('experience') || s.includes('employment') || s.includes('work') ||
  ...
  // Hard cap: zero quantified achievements → score can't exceed 5
  // (lowered from 7 — corpus data showed even this was too generous)
  if (totalMetrics === 0) expScore = Math.min(expScore, 5);

  return {
    score: Math.min(expScore, 25),
    weakBullets, issues, verbRatio, metricRatio, totalMetrics,
...
function computeFormattingScore(parsed) {
  ...
  // Contact
  if (!parsed.contact.email)      { score -= 5; issues.push('No email address detected.'); }
  else if (!parsed.contact.phone) { score -= 2; issues.push('No phone number detected.'); }
  ...
  if (!hasExp)    { score -= 6; issues.push('No experience section found.'); }
  if (!hasEdu)    { score -= 3; issues.push('No education section found.'); }
  if (!hasSkills) { score -= 3; issues.push('Add a dedicated "Technical Skills" section.'); }
  if (Object.keys(parsed.sections).length < 3) { score -= 3; issues.push('Very few sections detected — resume may be unstructured.'); }

  // Resume length
  const wc = parsed.stats.wordsCount;
  if      (wc < 200) { score -= 5; issues.push(`Resume too short (${wc} words) — aim for 350–750 words.`); }
  else if (wc < 300) { score -= 3; issues.push(`Resume is thin (${wc} words) — add more detail to experience.`); }
  else if (wc > 1100){ score -= 4; issues.push(`Resume too long (${wc} words) — trim to one page.`); }
  else if (wc > 850) { score -= 2; issues.push(`Resume is slightly long (${wc} words) — consider trimming.`); }

  // Bullet quality
  const avgLen = parsed.bullets.length
    ? parsed.bullets.reduce((a, b) => a + b.text.split(' ').length, 0) / parsed.bullets.length
    : 0;
  if (avgLen > 40)                          { score -= 2; issues.push('Bullets are too long — keep each under 20 words.'); }
  if (parsed.bullets.length < 3 && hasExp)  { score -= 3; issues.push('Too few bullet points — add detail under each role.'); }

  // Generic filler
  const FILLER = [
    'team player','hard-working','hardworking','detail-oriented','self-motivated',
    'passionate about','fast learner','quick learner','excellent communication',
    'good communication','problem solver','results-driven','results driven',
    'good knowledge','good understanding','basic knowledge','exposure to',
  ...
  if      (fillerHits >= 4) { score -= 3; issues.push('Many generic phrases ("familiar with", "passionate about") — replace with specific achievements.'); }
  else if (fillerHits >= 2) { score -= 1; issues.push('Some generic phrases detected — "familiar with" and "knowledge of" weaken ATS ranking.'); }

  // Date completeness
  const expLines  = (parsed.sections['experience'] || parsed.sections['work experience'] || []).join(' ');
  const hasDate   = /\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s\-,]+\d{4}\b/gi.test(expLines)
                  || /\b(20\d{2}|19\d{2})\b/.test(expLines);
  if (hasExp && !hasDate) { score -= 2; issues.push('No dates detected in experience — add start/end dates for each role.'); }
...
function computeSkillsSectionScore(parsed, kwInfo, wordCount, totalMetrics) {
  const lowerSecs = Object.keys(parsed.sections).map(s => s.toLowerCase());
  const hasSection = lowerSecs.some(s =>
    s.includes('skill') || s.includes('technolog') || s.includes('competenc') || s.includes('tools')
  );
  const distinct = kwInfo.distinctCount;

  let base;
  if      (distinct >= 22) base = 11;
  else if (distinct >= 17) base = 9;
  else if (distinct >= 13) base = 7;
  else if (distinct >= 9)  base = 5;
  else if (distinct >= 5)  base = 3;
  else                     base = 1;

  let skillsScore = base + (hasSection ? 2 : 0);
  if (!hasSection) skillsScore = Math.min(skillsScore, 6);

  // Thin + no metrics: skills are undemonstrated claims
  if (wordCount < 300 && totalMetrics === 0) skillsScore = Math.min(skillsScore, 6);

  return { score: Math.min(skillsScore, 15), hasSection, distinct };
}
...
function buildSampleImprovements(primaryDomain, expInfo) {
  const examples = {
    systems_programming: [
      { before: 'Built a server using C++ and sockets.', after: 'Engineered a multi-threaded Winsock2 TCP server in C++ handling 500+ concurrent connections with sub-10ms average latency.' },
      { before: 'Optimized memory usage.', after: 'Reduced peak heap allocation by 35% via memory pool reuse and RAII patterns, eliminating 12 classes of memory leaks.' },
    ],
    data_science: [
      { before: 'Trained a machine learning model.', after: 'Trained a Random Forest classifier achieving 94.3% accuracy on a 50K-sample dataset, outperforming SVM baseline by 8.7 percentage points.' },
      { before: 'Worked on data pipelines.', after: 'Designed an Apache Airflow ETL pipeline ingesting 3M+ daily events, reducing manual processing by 6 hours/week.' },
    ],
    web_backend: [
      { before: 'Built REST APIs for the platform.', after: 'Engineered 14 RESTful API endpoints in Node.js/Express with JWT auth, reducing average response time by 42% through Redis caching.' },
      { before: 'Improved application performance.', after: 'Optimized 8 PostgreSQL queries via composite indexing, cutting p95 latency from 820ms to 145ms.' },
    ],
    web_frontend: [
      { before: 'Created the UI for the application.', after: 'Architected a React component library (32 components) with TypeScript, reducing UI dev time by 40% across 3 product teams.' },
      { before: 'Improved page load speed.', after: 'Optimized bundle size by 55% via code splitting and lazy loading, improving First Contentful Paint from 3.8s to 1.4s.' },
    ],
    cloud_devops: [
      { before: 'Set up the CI/CD pipeline.', after: 'Built a GitHub Actions CI/CD pipeline with Docker builds and automated tests, cutting deployment time from 28 to 6 minutes.' },
      { before: 'Helped with Kubernetes deployment.', after: 'Migrated 5 microservices to Kubernetes on AWS EKS, achieving 99.95% uptime and reducing infra costs by $1,200/month.' },
    ],
    default: [
      { before: 'Responsible for building a web app using React.', after: 'Developed a responsive React web application improving user engagement by 28% and reducing support tickets by 15% post-launch.' },
      { before: 'Worked on backend APIs.', after: 'Designed and deployed 10+ RESTful APIs processing 50K+ daily requests with 99.9% uptime over a 3-month production window.' },
    ],
  };
  ...
  if (actual) {
    return [
      { before: actual.slice(0, 120), after: 'Start with a power verb + specific technology + measurable outcome. E.g.: "Engineered [what] using [tech], achieving [X% improvement / N users]."' },
      pool[0],
      pool[1] || fallback[0],
    ];
  }
...
function calculateATSScore(input, jobDescriptionText = null) {
  ...
  // ── Unreadable / image PDF detection ─────────────────────────────────────
  // If almost no text was extracted, the PDF is almost certainly image-based.
  // Give a specific, actionable message instead of the generic "not a resume" one.
  if (parsed.stats.wordsCount < 15) {
    return {
      totalScore: 0,
      ...
      freeAnalysis: {
        ...
        weakBullets: [],
        formattingIssues: [
          'Could not extract text from this PDF. This is usually a scanned or image-based PDF.',
          'Fix: export your resume as a text-based PDF directly from Word, Google Docs, or Overleaf. Do not upload a screenshot or photo of your resume.',
        ],
        sampleImprovements: [],
      },
    };
    ...
    return {
      ...
      freeAnalysis: {
        ...
        formattingIssues: [
          ...
          'Please upload a CV or resume file in PDF or DOCX format.',
        ],
        sampleImprovements: [],
      },
    };
  ...
  const lowerSecs = Object.keys(parsed.sections).map(s => s.toLowerCase());
  const hasExpSection = lowerSecs.some(s =>
    s.includes('experience') || s.includes('employment') || s.includes('work')
  );
  const hasProjectsSection = lowerSecs.some(s =>
    s.includes('project') || s.includes('portfolio')
  );
  ...
  const totalPenalty   = buzzwordInfo.penalty + weakVerbInfo.penalty +
                         growthInfo.penalty + repetitionInfo.penalty;
  ...
  const incompletenessSignals = [
    ...
    hasExpSection && (() => {                           // experience section exists but no dates
      const expLines = (parsed.sections['experience'] || parsed.sections['work experience'] || []).join(' ');
      return !/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s\-,]+\d{4}\b/gi.test(expLines)
          && !/\b(20\d{2}|19\d{2})\b/.test(expLines);
    })(),
  ...
  const compoundBase = incompletenessSignals >= 4 ? 10
                     : incompletenessSignals >= 3 ? 6
                     : incompletenessSignals >= 2 ? 2
                     : 0;
  ...
  // Corpus testing showed ResumeIQ systematically scores ~23pts above ResumeWorded.
  // This multiplier corrects the distribution without breaking relative ordering.
  // Floor of 5 preserved so no resume scores 0 unless isLikelyResume() rejects it.
  // Worstcase (IQ 26) after multiplier → ~22, which is appropriate for that resume.
  // ── Non-linear calibration (corpus-derived, Nov 2025) ────────────────────
  // Flat multipliers over-penalise strong resumes and under-penalise weak ones.
  // These bands were tuned against 10 resumes benchmarked on ResumeWorded.
  // Real-PDF avg delta: ~4 pts. OCR resumes score higher due to cleaner parse —
  // that's a known limitation, not a scoring error.
  const calibrate = (s) =>
    s >= 75 ? Math.round(s * 0.92) :
    s >= 60 ? Math.round(s * 0.93) :
    s >= 45 ? Math.round(s * 0.92) :
    s >= 30 ? Math.round(s * 0.95) : s;

  totalScore = Math.max(5, calibrate(totalScore));

  // ── Penalty issues for UI ─────────────────────────────────────────────────
  const penaltyIssues = [];
  if (buzzwordInfo.count >= 2)
    penaltyIssues.push(`${buzzwordInfo.count} buzzwords detected ("${buzzwordInfo.found.slice(0, 2).join('", "')}") — replace with specific achievements.`);
  ...
  if (repetitionInfo.repeatedPhrases.length >= 2)
    penaltyIssues.push(`Repeated phrases across bullets ("${repetitionInfo.repeatedPhrases[0]}") — vary language to avoid sounding formulaic.`);
  ...
  if (jobDescriptionText) {
    ...
    let matchCount = 0;
    jdToks.forEach(t => { if (resToks.has(t)) matchCount++; });
    jdCoverage = {
